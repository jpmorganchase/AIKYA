...
from src.ml_models.tool.model_builder_tool import simulate_feedback_is_correct, generate_labels_array
...
class CreditCardSimulationExecutor(SimulationModelExecutor):
    """
    ...
    def label(self, predictions):
        ...
        try:
            ...
            predict_data = self.simulate_feedback_is_correct(predict_data)
            is_correct_req = [entry['is_correct_req'] for entry in predict_data]
            result_list = [entry['result'] for entry in predict_data]
            # TODO: Review THIS LOGIC
            threshold = 73.0
            labels_array = generate_labels_array(is_correct_req, result_list, threshold)
            return labels_array
        except Exception as e:
